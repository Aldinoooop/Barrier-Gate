...
void loop() {  // run over and over
  if (mySerial.available() > 0) {
    command = "";
    kata = mySerial.readStringUntil('\n');
    Serial.println(String(kata));
    int separatorIndex = kata.indexOf('#');
    if (separatorIndex != -1) {                     // Jika '#' ditemukan
      command = kata.substring(0, separatorIndex);  // Mengambil substring sebelum '#'
      waktu = kata.substring(separatorIndex + 1);   // Mengambil substring setelah '#'

      // Menampilkan hasil
      Serial.print("Kata: ");
      Serial.println(command);  // Menampilkan bagian kata
      Serial.print("Waktu: ");
      Serial.println(waktu);  // Menampilkan bagian waktu
    } else {
      Serial.println("Separator '#' tidak ditemukan.");
    }
  }
  delay(1000);
  ...
  if (String(command) == String("Capture1")) {
    ...
    saved = 0;
    if (CAM3_EXIST) {
      myCAMSaveToSDFile(myCAM3, "3");
      // myCAMSaveToSDFile(myCAM3, String(waktu + "in"));
      // Serial.println("Cam1 Captured");
    }
    // delay(1000);
    if (CAM4_EXIST) {
      // Serial.println(String(waktu + "out"));
      // myCAMSaveToSDFile(myCAM4, String(waktu + "out"));
      myCAMSaveToSDFile(myCAM4, "4");
...
void myCAMSaveToSDFile(ArduCAM myCAM, String taim) {
  char str[8];
  byte buf[256];
  static int i = 0;
  static int k = 0;
  ...
  itoa(k, str, 10);
  strcat(str, ".jpg");
  // strcpy(str, taim.c_str());  // Menyalin string waktu ke str
  // strcat(str, ".jpg");         // Menggabungkan dengan ".jpg"
  // Serial.println(str);
  //Open the new file
